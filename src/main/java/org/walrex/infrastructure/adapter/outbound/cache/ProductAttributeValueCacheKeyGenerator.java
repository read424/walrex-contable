...
/**
 * Generador de claves únicas para caché de valores de atributos de producto.
 *
 * Genera claves basadas en:
 * - ProductAttributeValueFilter (todos los campos incluyendo attributeId)
 * - Número de página
 * - Tamaño de página
 * - Campo de ordenamiento
 * - Dirección de ordenamiento
 */
public class ProductAttributeValueCacheKeyGenerator {

    private static final String CACHE_PREFIX = "product-attribute-value:list:";
    private static final String CACHE_ALL_PREFIX = "product-attribute-value:all:";
    private static final String CACHE_BY_ATTRIBUTE_PREFIX = "product-attribute-value:by-attribute:";
    private static final ObjectMapper objectMapper = new ObjectMapper();

    /**
     * Genera una clave única de caché para una consulta de listado.
     *
     * Formato: product-attribute-value:list:{hash}
     ...
     */
    public static String generateKey(PageRequest pageRequest, ProductAttributeValueFilter filter) {
        try {
            ...
            CacheKeyComponents components = new CacheKeyComponents(
                ...
                filter != null && filter.getAttributeId() != null ? filter.getAttributeId().toString() : null,
     ...
     *
     * Formato: product-attribute-value:all:{hash}
     * donde {hash} es un SHA-256 de todos los parámetros serializados.
     *
     * @param filter Filtros aplicados (puede ser null)
     * @return Clave única para esta combinación de parámetros
     */
    public static String generateKey(ProductAttributeValueFilter filter) {
        try {
            // Crear un objeto que combine todos los parámetros
    ...
    /**
     * Genera clave para invalidar todo el patrón de valores de atributos.
     * Invalida tanto claves paginadas (product-attribute-value:list:*)
     * como no paginadas (product-attribute-value:all:*)
     * como por atributo (product-attribute-value:by-attribute:*).
     *
     * @return Patrón para invalidar todas las claves de valores de atributos
     */
     ...
     */
    private static String buildFallbackKey(PageRequest pageRequest, ProductAttributeValueFilter filter) {
        ...
        if (filter != null) {
            ...
            if (filter.getAttributeId() != null) {
                key.append(":attributeId:").append(filter.getAttributeId().toString());
            }
     ...
     * Construye clave alternativa si falla la serialización JSON.
     */
    private static String buildFallbackKeyForAll(ProductAttributeValueFilter filter) {
        StringBuilder key = new StringBuilder();
